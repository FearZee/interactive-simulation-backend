...
from database.device.device_service import get_controlled_devices
from database.schedule.schedule_service import (
    get_schedule_by_reference,
    logic,
    get_schedule_by_reference_with_devices,
    get_schedule_with_usage,
)
...
@schedule_router.get("/schedule/controlled_devices")
async def get_controlled_devices_query(db=Depends(get_db)):
    return get_controlled_devices(db=db)
...
async def get_schedule_usage_by_reference(
    schedule_reference: uuid.UUID, db=Depends(get_db)
):
